...
チャットのツールバーから Lumii にログインしてチャンネルを自動設定したり、同じ場所からログアウトしたりできます（チャンネルページを開かなくても構いません）。
...
Lumii チャンネル用プラグイン（`@wemotechnology/openclaw-lumii`）も同梱され、起動時に他のパッケージ済みチャンネルプラグインと同様に `~/.openclaw/extensions/openclaw-lumii` へ自動インストールまたは更新されます。
